...
import { AcaoProjeto, CreateProject1Dto, UpdateProject1Dto } from "@/types/api";
...
export class ProjectService {
  ...
  async getById(id: string): Promise<AcaoProjeto> {
    try {
      const response = await api.get<AcaoProjeto>(
  ...
  async create(data: CreateProject1Dto): Promise<AcaoProjeto> {
    try {
      const response = await api.post<AcaoProjeto>(
  ...
  async update(id: string, data: UpdateProject1Dto): Promise<AcaoProjeto> {
    try {
      const response = await api.put<AcaoProjeto>(
        `${API_ENDPOINTS.PROJECTS}/${id}`,
        data,
      );
      return response.data;
